...
export function mergeProductInfo(item: PopItem, result: ProductInfoResponse, replaceIdentity = false): PopItem {
  ...
  return {
    ...
    sku: replaceIdentity ? suggestion.sku || item.sku : item.sku || suggestion.sku,
    upc: replaceIdentity ? suggestion.upc || item.upc : item.upc || suggestion.upc,
    description: replaceIdentity ? suggestion.description || item.description : item.description || suggestion.description,
    releaseDate: replaceIdentity ? suggestion.releaseDate || item.releaseDate : item.releaseDate || suggestion.releaseDate,
